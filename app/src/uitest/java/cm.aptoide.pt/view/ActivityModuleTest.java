...
/**
 * Created by jose_messejana on 05-12-2017.
 */

@Module public class ActivityModuleTest {
  ...
  public ActivityModuleTest(AppCompatActivity activity, Intent intent, NotificationSyncScheduler notificationSyncScheduler, String marketName,
  ...
  @ActivityScopeTest @Provides ApkFy provideApkFy(
      @Named("secureShared") SharedPreferences securePreferences) {
    return new ApkFy(activity, intent, securePreferences);
  }

  @ActivityScopeTest @Provides AutoUpdate provideAutoUpdate(DownloadFactory downloadFactory,
  ...
  @ActivityScopeTest @Provides FragmentNavigator provideFragmentNavigator(
  ...
  @ActivityScopeTest @Provides FragmentManager provideFragmentManager() {
    return activity.getSupportFragmentManager();
  }

  @ActivityScopeTest @Provides DeepLinkManager provideDeepLinkManager(StoreUtilsProxy storeUtilsProxy,
      ...
      @Named("default") OkHttpClient httpClient, @Named("pool-v7") BodyInterceptor<BaseBody> bodyInterceptorV7,
  ...
  @ActivityScopeTest @Provides
  Presenter provideMainPresenter(RootInstallationRetryHandler rootInstallationRetryHandler,
  ...
  @ActivityScopeTest @Provides AccountNavigator provideAccountNavigator(FragmentNavigator fragmentNavigator, AptoideAccountManager accountManager,
  ...
  @ActivityScopeTest @Provides ScreenOrientationManager provideScreenOrientationManager(){
    return new ScreenOrientationManager(activity, (WindowManager) activity.getSystemService(WINDOW_SERVICE));
  }

  @ActivityScopeTest @Provides AccountPermissionProvider provideAccountPermissionProvider(){
    return new AccountPermissionProvider(((PermissionProvider) activity));
  }

  @ActivityScopeTest @Provides PhotoFileGenerator providePhotoFileGenerator(){
    return new PhotoFileGenerator(activity,
        activity.getExternalFilesDir(Environment.DIRECTORY_PICTURES), fileProviderAuthority);
  }

  @ActivityScopeTest @Provides UriToPathResolver provideUriToPathResolver(){
    return new UriToPathResolver(activity.getContentResolver());
  }

  @ActivityScopeTest @Provides ImagePickerNavigator provideImagePickerNavigator(){
  ...
  @ActivityScopeTest @Provides ManageStoreNavigator provideManageStoreNavigator(FragmentNavigator fragmentNavigator){
