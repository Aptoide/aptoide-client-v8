...
public class AppViewFragment extends AptoideBaseFragment<BaseAdapter>
  ...
  @Override public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
    ...
    getLifecycle().filter(lifecycleEvent -> lifecycleEvent.equals(LifecycleEvent.CREATE))
        .flatMap(viewCreated -> accountNavigator.notLoggedInViewResults(LOGIN_REQUEST_CODE)
            ...
            .flatMapCompletable(result -> socialRepository.asyncShare(getPackageName(),
                appViewModel.getStoreId(), "app")
    ...
    if (getAppId() >= 0) {
      ...
      subscription = appRepository.getAppFromUname(getUname(), refresh, isSponsored())
          ...
          .subscribe(getApp -> {
            ...
            finishLoading(throwable);
            CrashReport.getInstance().log(key_appId, String.valueOf(getAppId()));
            CrashReport.getInstance().log(key_packageName, String.valueOf(getPackageName()));
            CrashReport.getInstance().log(key_uname, getUname());
          });
    } else {
  ...
  @Override public ScreenTagHistory getHistoryTracker() {
    return ScreenTagHistory.Builder.build(this.getClass().getSimpleName(),
        appViewModel.getOriginTag(), null);
  }
  ...
  public void buyApp(GetAppMeta.App app) {
    ...
    startActivityForResult(PaymentActivity.getIntent(getActivity(),
        ...
        getBillingIdResolver().resolveStoreSellerId(app.getStore().getName()), null),
  ...
  @Override public boolean onOptionsItemSelected(MenuItem item) {
    ...
    if (i == R.id.menu_share) {
      ...
      final boolean appRatingExists = getApp() != null
          ...
          && getApp().getStats().getRating() != null;

      final float averageRating = appRatingExists ? getApp().getStats().getRating().getAvg() : 0f;

      final boolean appHasStore = getApp() != null && getApp().getStore() != null;

      final Long storeId = appHasStore ? getApp().getStore().getId() : null;
  ...
  @NonNull private Observable<GetApp> setKeywords(GetApp getApp) {
    this.appViewModel.setKeywords(getApp.getNodes().getMeta().getData().getMedia().getKeywords());

    return Observable.just(getApp);
  }

  private void setupAppView(GetApp getApp) {
    this.appViewModel.setApp(getApp.getNodes().getMeta().getData());

    List<Group> groupsList = getApp.getNodes().getGroups().getDataList().getList();
    ...
    if (getStoreTheme() == null) {
      setStoreTheme(getApp.getNodes().getMeta().getData().getStore().getAppearance().getTheme());
    }

    // useful data for the syncAuthorization updates menu option
    installAction(getPackageName(), getApp().getFile().getVercode()).observeOn(
        ...
        .subscribe(appAction -> {
          ...
          if (appAction != AppAction.INSTALL) {
            setUnInstallMenuOptionVisible(() -> new PermissionManager().requestDownloadAccess(
                ...
                .flatMap(success -> installManager.uninstall(getPackageName(),
                    getApp().getFile().getVername()).toObservable())
                .compose(bindUntilEvent(FragmentEvent.DESTROY_VIEW))
                .subscribe(aVoid -> {
  ...
  private void updateLocalVars(GetAppMeta.App app) {
    ...
    setStoreName(app.getStore().getName());
    this.appViewModel.setStoreId(app.getStore().getId());
    if (getStoreTheme() == null) {
      setStoreTheme(app.getStore().getAppearance().getTheme());
    }
    setMd5(app.getMd5());
  ...
  public void setupShare(GetApp app) {
    setAppName(app.getNodes().getMeta().getData().getName());
    this.appViewModel.setwUrl(app.getNodes().getMeta().getData().getUrls().getW());
  }
  ...
  public void showSuggestedApps() {
    ...
    adsRepository.getAdsFromAppviewSuggested(getPackageName(), appViewModel.getKeywords())
        ...
        .zipWith(requestFactoryCdnWeb.newGetRecommendedRequest(6, getPackageName()).observe(),
  ...
  private final class AppViewHeader {
    ...
    public void setup(@NonNull GetApp getApp) {
      ...
      appViewAnalytics.sendAppViewOpenedFromEvent(aptoideNavigationTracker.getPreviousScreen(),
          aptoideNavigationTracker.getCurrentScreen(), getPackageName(),
