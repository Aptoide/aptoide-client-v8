...
import cm.aptoide.pt.feature_gamegenie.data.database.GameCompanionDao
...
import cm.aptoide.pt.feature_gamegenie.domain.toToken
...
import cm.aptoide.pt.feature_gamegenie.io_models.toDomain
  ...
  suspend fun postMessage(
    ...
    return try {
      ...
      if (e.code() == 401) {
        ...
        gameGenieApi.postMessage("Bearer ${newToken.token}", request)
      } else {
        throw e
      }
    }
  ...
  suspend fun postMessageCompanion(
    ...
    return try {
      ...
      if (e.code() == 401) {
        Timber.i("Token expired, requesting a new token")
        val newToken = fetchNewToken()
        gameGenieApi.postMessageCompanion("Bearer ${newToken.token}", request)
      } else {
        throw e
      }
    }
