...
public class DateCalculator {
  ...
  public String getTimeSinceDate(Context context, Date date) {
	...
	public DateCalculator() {
		mTimestampLabelYesterday = V8Engine.getContext().getResources()
				.getString(cm.aptoide.pt.utils.R.string.WidgetProvider_timestamp_yesterday);
		mTimestampLabelToday = V8Engine.getContext().getResources().getString(cm.aptoide.pt.utils.R.string.WidgetProvider_timestamp_today);
		mTimestampLabelJustNow = V8Engine.getContext().getResources().getString(cm.aptoide.pt.utils.R.string.WidgetProvider_timestamp_just_now);
		mTimestampLabelMinutesAgo = V8Engine.getContext().getResources()
				.getString(cm.aptoide.pt.utils.R.string.WidgetProvider_timestamp_minutes_ago);
		mTimestampLabelHoursAgo = V8Engine.getContext().getResources().getString(cm.aptoide.pt.utils.R.string
				.WidgetProvider_timestamp_hours_ago);
		mTimestampLabelHourAgo = V8Engine.getContext().getResources().getString(cm.aptoide.pt.utils.R.string.WidgetProvider_timestamp_hour_ago);
		mTimestampLabelDaysAgo = V8Engine.getContext().getResources().getString(cm.aptoide.pt.utils.R.string.WidgetProvider_timestamp_days_ago);
		mTimestampLabelWeekAgo = V8Engine.getContext().getResources().getString(cm.aptoide.pt.utils.R.string.WidgetProvider_timestamp_week_ago2);
		mTimestampLabelWeeksAgo = V8Engine.getContext().getResources().getString(cm.aptoide.pt.utils.R.string
				.WidgetProvider_timestamp_weeks_ago);
		mTimestampLabelMonthAgo = V8Engine.getContext().getResources().getString(cm.aptoide.pt.utils.R.string
				.WidgetProvider_timestamp_month_ago);
		mTimestampLabelMonthsAgo = V8Engine.getContext().getResources()
				.getString(cm.aptoide.pt.utils.R.string.WidgetProvider_timestamp_months_ago);
		mTimestampLabelYearAgo = V8Engine.getContext().getResources().getString(cm.aptoide.pt.utils.R.string.WidgetProvider_timestamp_year_ago);
		mTimestampLabelYearsAgo = V8Engine.getContext().getResources().getString(cm.aptoide.pt.utils.R.string
	...
	private String getTimeDiffAll(Context context, long time) {
		...
		if (isYesterday(time) || DateUtils.isToday(time)) {
			...
			if (diffTime < DateUtils.WEEK_IN_MILLIS) {
				...
				int diffDays = Double.valueOf(Math.ceil(diffTime / AptoideUtils.DateTimeU.WEEK_IN_MILLIS)).intValue();
				...
				int diffDays = Double.valueOf(Math.ceil(diffTime / (AptoideUtils.DateTimeU.WEEK_IN_MILLIS * 4))).intValue();
				...
				int diffDays = Double.valueOf(Math.ceil(diffTime / (AptoideUtils.DateTimeU.WEEK_IN_MILLIS * 4 * 12))).intValue();
