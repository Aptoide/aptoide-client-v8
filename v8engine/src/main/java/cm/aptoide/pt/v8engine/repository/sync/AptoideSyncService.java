    ...
    synchronized (lock) {
      if (syncAdapter == null) {
        syncAdapter = new AptoideSyncAdapter(getApplicationContext(), true, false,
            RepositoryFactory.getPaymentAuthorizationRepository(getApplicationContext()),
            new PaymentConfirmationConverter(), new PaymentAuthorizationConverter(),
            new ProductBundleConverter(), new NetworkOperatorManager(
