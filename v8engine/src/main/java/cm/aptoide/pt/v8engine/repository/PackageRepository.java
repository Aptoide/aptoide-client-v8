...
public class PackageRepository {
	...
	public Observable<String> getRandomInstalledPackages(int count) {
		return getInstalledPackages().map(packageInfos -> {
			...
			return packageInfos;
		}).flatMapIterable(packageInfos -> packageInfos).take(count).map(packageInfo -> packageInfo.packageName).subscribeOn(Schedulers.io());
	}

	@NonNull
	private Observable<List<PackageInfo>> getInstalledPackages() {
		return getCachedInstalledPackages()
				.onErrorResumeNext(getPackageManagerInstalledPackages())
