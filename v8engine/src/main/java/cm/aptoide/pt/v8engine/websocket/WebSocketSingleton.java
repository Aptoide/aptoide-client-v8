...
/**
 * Created with IntelliJ IDEA. User: brutus Date: 30-09-2013 Time: 11:33 To change this template use File | Settings |
 * File Templates.
 */
public class WebSocketSingleton {
	...
	String[] matrix_columns = new String[]{SearchManager.SUGGEST_COLUMN_ICON_1, SearchManager.SUGGEST_COLUMN_TEXT_1, SearchManager.SUGGEST_COLUMN_QUERY, "_id"};
	...
	public void send(final String query) {
		this.query = query;
		// Fix nullPointer
		if (web_socket_client != null && web_socket_client.isConnected() && query.length() > 2 && (buffer == null ||
                !query
				.startsWith(buffer))) {
	...
	public void connect() {
		...
		if (web_socket_client == null) {
			web_socket_client = new WebSocketClient(java.net.URI.create("ws://buzz.webservices.aptoide.com:9000"),
                    listener, null);
