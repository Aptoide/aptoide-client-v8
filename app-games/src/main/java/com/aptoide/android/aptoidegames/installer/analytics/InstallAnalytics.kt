...
class InstallAnalytics(
    ...
    genericAnalytics.logEvent(
      ...
      params = analyticsContext.toGenericParameters(
        *app.toGenericParameters(),
        P_UPDATE_TYPE to getUserClicks(app.packageName) + 1,
  ...
  fun sendOpenClick(
    ...
    genericAnalytics.logEvent(
      ...
      params = analyticsContext.toGenericParameters(
        ...
        P_UPDATE_TYPE to getUserClicks(packageName) + 1
      )
    )
  }
  ...
  private fun getUserClicks(packageName: String): Int = silentInstallChecker
  ...
  this?.run {
    toAnalyticsUiContext().toGenericParameters(
      *pairs,
      ...
      P_UPDATE_TYPE to if (userClicks == 1) "1-tap" else "$userClicks-taps"
    )
  } ?: mapOfNonNull(*pairs)
