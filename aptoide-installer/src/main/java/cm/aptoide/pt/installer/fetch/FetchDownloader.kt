...
class FetchDownloader @Inject constructor(
  ...
  private fun downloadFiles(
    ...
    val downloads = requests.map { it.id }.toMutableSet()
    val downloadsProgresses = requests.associate { it.id to 0L }.toMutableMap()
    ...
    return callbackFlow<Double> {
      ...
      listener = object : FetchFileDownloadListener {
        ...
        override fun onProgress(
          ...
          downloadsProgresses.replace(download.id, download.downloaded)
          trySend(downloadsProgresses.values.sum() / totalSize)
        }
      }
