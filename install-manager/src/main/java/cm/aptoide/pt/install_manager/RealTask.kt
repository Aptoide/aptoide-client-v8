...
internal class RealTask internal constructor(
  ...
  override val stateAndProgress: Flow<Task.State> = _stateAndProgress.transformWhile {
    emit(it)
    it::class.simpleName !in listOf(
      Task.State.Aborted::class.simpleName,
      Task.State.Canceled::class.simpleName,
      Task.State.Completed::class.simpleName,
      Task.State.OutOfSpace::class.simpleName,
      Task.State.Failed::class.simpleName
    )
  }
